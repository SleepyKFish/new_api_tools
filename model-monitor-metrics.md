...
| `input_tokens_total` 新口径 | ⚠️ 未使用。前端日志列表已经在使用 `other.input_tokens_total` 作为 OpenAI-like 输入总量,本监控分母没读它。当输入是文本+图片+音频混合时可能略低估,但 cache 比例计算上影响有限                                                       |
...
| `input_tokens_total` 新口径未使用(§4.4.5)     | 含图/音多模态时分母略低估                         | 若该字段普及,优先用作 OpenAI-like 分母                          |
...
NewAPI logs 表
   ...
   │
   └─[2] 应用层流式扫描(批 5000 行)
           └─ 仅 type=2 → 计算 TTFT/总耗时/tok-s/cache_hit
