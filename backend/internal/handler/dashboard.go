...
// GET /api/dashboard/trends/hourly/current
func GetCurrentHourlyTrend(c *gin.Context) {
	noCache := c.Query("no_cache") == "true"
	svc := service.NewDashboardService()

	data, err := svc.GetCurrentHourlyTrend(noCache)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": gin.H{"message": err.Error()}})
		return
	}
...
// GET /api/dashboard/trends/hourly/completed
func GetCompletedHourlyTrend(c *gin.Context) {
	startUnix, err := strconv.ParseInt(c.Query("start"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": gin.H{"message": "valid start timestamp is required"}})
		return
	}

	noCache := c.Query("no_cache") == "true"
	svc := service.NewDashboardService()
	data, err := svc.GetCompletedHourlyTrend(startUnix, noCache)
	if err != nil {
		status := http.StatusInternalServerError
		if errors.Is(err, service.ErrInvalidCompletedHour) {
